...
public extension Custard {
    /// カザフ語配列 (Extended Cyrillic with Ә, Ғ, Қ, Ң, Ө, Ұ, Ү, Һ, І)
    /// 特徴: ロシア語ベース + カザフ固有文字
    static let cyrillicKazakh = Custard(
        identifier: "cyrillic_kazakh",
        language: .ja_JP,
        input_style: .direct,
        metadata: .init(
        ...
        interface: CustardInterface(
            ...
            keys: [
                ...
                .gridFit(.init(x: 5, y: 2)): .custom(.input("і")), // І - Kazakh specific
