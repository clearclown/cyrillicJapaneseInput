    ...
    var body: some View {
        TipsContentView("フルアクセスについて") {
            TipsContentParagraph {
                Text("Pismoは、キーボードの振動フィードバックや、キーボードからのペーストを実現するために、フルアクセスの許可が必要です。フルアクセスを許可していただくことで、これらの機能が使用できます。")
            }
            TipsContentParagraph {
            ...
            TipsContentParagraph {
                ...
                Text("Pismoは安全なキーボードアプリであり、ユーザの明示的な同意なく、個人情報を外部に送信したり、保存したりすることはありません。")
                Text("なお、Pismoはオープンソースであり、誰もが実装を確認することができます。")
            }
            ...
            Section("参考情報") {
                FallbackLink("Pismoのソースコードを閲覧する", destination: URL(string: "https://github.com/clearclown/cyrillicJapaneseInput")!)
